document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('http://localhost:3000/api/products')
        ...
        .then(products => {
            ...
            const meatProducts = products.filter(product => product.category_id === 50);

            meatProducts.forEach(product => {
                const productCard = template.content.cloneNode(true);
                
                productCard.querySelector('.product-image').src = `../resources/images/fish_page/${product.name.toLowerCase().replace(/ /g, '_')}.png`;
