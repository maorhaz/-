document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('http://localhost:3000/api/products')
        ...
        .then(products => {
            ...
            const meatProducts = products.filter(product => product.category_id === 70);

            meatProducts.forEach(product => {
                const productCard = template.content.cloneNode(true);
                
                productCard.querySelector('.product-image').src = `../resources/images/cheese_page/${product.name.toLowerCase().replace(/ /g, '_')}.png`;
